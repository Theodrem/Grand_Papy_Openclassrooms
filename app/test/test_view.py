    ...
    def test_process(self, mock_geo, mock_wiki):
        ...
        with app.test_client() as c:
            rv = c.post('/process', json={'input_user': p.transform_input(), 'lat': self.lat, 'lng': self.lng,
                                          ...
                                          'end_mess': end_message,
                                          'wiki': WIKI_DATA_DESC["query"]["pages"][str(10142861)]["extract"]})
            ...
            mock_wiki.return_value.get_description.return_value = WIKI_DATA_DESC["query"]["pages"][str(10142861)][
                "extract"]
